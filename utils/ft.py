...
def ft_sh_phase_screen_ea(Uin, phz_params_dict, genetic_code=None):
...
def ift_sh_phase_screen_ea(Uout, N=512, Lout=10, Lin=1e-3, deltax=0.005, wvl=0.532e-6, Dz=50e3, nscreen=50, kpow=22/6, Rytov=0.2, Np=5, genetic_code=None):
    k=2*pi/wvl
    nn=np.arange(-np.floor(N/2), np.floor(N/2))
    nx, ny = np.meshgrid(nn, nn)
    nsq = nx**2 + ny**2
    ...
    for idx in range(nscreen-1,0,-1):
        #r0 = (.423*(k**2)*Cn2*deltaz)**(-3/5) # Constant Cn2
        ...
        g = exp(1j/phz) / g
        Q2 = exp(1j*pi**2*2*deltaz/k*fsq)
        G = Q2/ifftshift(ifft2(ifftshift(g)))/deltax**2 #might be deltaf
        g = fftshift(fft2(fftshift(G)))/(N*deltaf)**2
        #g = exp(1j/phz)
